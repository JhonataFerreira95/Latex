...
- Aqui irei aborda a conexão do nosso arquivo `.bib` com nosso texto em `LateX`:
    ...
    - Primeiro vamos começar pelo comando `\bibliographystyle{plain}`, que utilizamos para definir o estilo da referência. O comando `\bibliography{references}` é para referênciar o arquivo com a extensão `.bib`. Para citar nosso `textbook` usamos o comando `\cite{textbook}` para citar nossas refeências. Também posso usar o comando `\nocite{*}` para deixarmos sem referência alguma. 
